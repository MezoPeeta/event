...
from apps.payment.paymob import paymob_iframe
...
class ProductListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            search_query = False
        if search_query:
            products = Products.objects.filter(name__icontains=search_query).order_by(
                "-id"
            )
        else:
        ...
        order_count = OrderItem.objects.filter(
            order__customer=get_customer(self.request), order__complete=False
        ).count()
...
def products(request, pk):
    ...
    if request.method == "POST":
        ...
        order, _ = Order.objects.get_or_create(
            customer=get_customer(request), complete=False
        )
